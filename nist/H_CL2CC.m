function [c,rc,f] = H_CL2CC(M)
    %
    % Implementation of a heuristic
    % to fit a constrained (circumscribed) least-squares circle in a plane
    % Input:
    ...
    % CL2CC_ObjFun: a local function needed by fminsearch
    %
    ...
    [c, f] = fminsearch(@CL2CC_ObjFun, x0); % Conduct the heuristic search
                                            % Input:
                                            % @CL2CC_ObjFun: handle for a local objective function
                                            % x0: a 1x2 array of starting circle center coordinates
                                            % Output:
                                            % c: a 1x2 array of circle center coordinates
                                            % f: minimized objective function; a scalar
    rc = max(sqrt((P(:,1)-c(1)).^2 + (P(:,2)-c(2)).^2)); % Radius of the circle
end
%
function f=CL2CC_ObjFun(x)
    ...
    f = (norm(r-max(r)))/sqrt(size(r)(1)); % RMS of deviations from the circle
end
